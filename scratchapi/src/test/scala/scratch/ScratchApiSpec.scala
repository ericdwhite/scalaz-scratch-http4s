...
class CajaSpec extends org.specs2.mutable.Specification {
  "HelloWorld" >> {
    "return 200" >> {
      uriReturns200()
    }
  ...
  private[this] val retHelloWorld: Response[Task] = {
    val getHW = Request[Task](Method.GET, Uri.uri("/hello/world"))
    new HelloWorldService[Task].service.orNotFound(getHW).unsafeRunSync()
  }
